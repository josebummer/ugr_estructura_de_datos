...
bool mutacion::operator==(const mutacion & m) const{
	bool iguales = true;
	if (this != &m ){
		if( this->ID != m.ID || this->chr != m.chr || this->genes != m.genes || this->enfermedades != m.enfermedades || this->pos != m.pos || 			this->ref_alt != m.ref_alt || this->common != m.common || this->caf != m.caf || this->clnsig != m.clnsig ){
			iguales = false;
		}
	}
	return iguales;
